...
'@))
    ...
    Check 'a skipped build is not reported as a pass' ($out -notmatch '\[PASS\] build') $out

    # #99: a HintPath can be GENERATED by a pre-build target -- JotunnLib's publicized
    # assemblies are created by its own task from AfterTargets="BeforeBuild" -- so a file
    # that is not there yet is no proof the game is absent. Reported from the field: the
    # gate said "game/SDK not installed" where plain `dotnet build` succeeded, and one
    # manual build flipped the verdict, which made it depend on build history. Same
    # fixture as above, plus a prebuild target that produces ..\lib: the project must be
    # BUILT and checked. The skip stays for a tree the build really cannot resolve --
    # that is the check right above this one.
    $pb = Join-Path $tmp 'prebuild'
    Copy-Item (Join-Path $PSScriptRoot 'testdata\harmony-fixture') $pb -Recurse
    $pbMod = Join-Path $pb 'Mod'
    $pbProj = Join-Path $pbMod 'Mod.csproj'
    [IO.File]::WriteAllText($pbProj, (Get-Content $pbProj -Raw).Replace('</Project>', @'
  <Target Name="GenRefs" AfterTargets="BeforeBuild">
    <Exec Command="dotnet build &quot;$(MSBuildThisFileDirectory)..\Game\Game.csproj&quot; -o &quot;$(MSBuildThisFileDirectory)..\lib&quot; -nologo -v q" />
    <Exec Command="dotnet build &quot;$(MSBuildThisFileDirectory)..\Harmony\Harmony.csproj&quot; -o &quot;$(MSBuildThisFileDirectory)..\lib&quot; -nologo -v q" />
  </Target>
</Project>
'@))
    $out = (& pwsh -NoProfile -File (Join-Path $PSScriptRoot 'gate\check.ps1') -Root $pbMod -All 2>&1 | Out-String)
    $pbCode = $LASTEXITCODE
    Check 'references a pre-build target generates are built, not skipped on build history' `
        (($pbCode -eq 0) -and ($out -match '\[PASS\] build') -and ($out -notmatch 'reference\(s\) missing')) "code=$pbCode $out"
...
# owns the tree; `qgate release` gives the hook back. Same failing tree throughout, so the
# only thing that changes the verdict is the hold. Fresh session ids: the block counter
# lives in TEMP keyed by (repo, session).
function Invoke-Qgate([string[]]$Argv) {
    (& cmd /c "`"$(Join-Path $PSScriptRoot 'bin\qgate.cmd')`" $($Argv -join ' ')" 2>&1 | Out-String)
}
...
Set-GoFile (Join-Path $holdRepo 'bad.go') "package main`nfunc  Bad() {}"
$h = Invoke-StopHook $holdRepo ([guid]::NewGuid())
Check 'stop-hook blocks a failing tree while no hold is set' ($h.Code -eq 2) "code=$($h.Code) $($h.Err)"
# The file was just written, so the blocking message must also say what to do when the
# writer is someone else. The message is a hint, never an excuse: the code is still 2.
Check 'the block names the hold as the way out when a file was written seconds ago' `
    ($h.Err -match 'qgate hold') $h.Err
$holdOut = Invoke-Qgate @('hold', '-Root', "`"$holdRepo`"")
Check 'qgate hold reports the window and that commits stay gated' `
    (($holdOut -match 'Stop hook off') -and ($holdOut -match 'commits are still gated')) $holdOut
Check 'qgate hold status reports an active hold' ((Invoke-Qgate @('hold', 'status', '-Root', "`"$holdRepo`"")) -match 'hold active') $holdOut
$h = Invoke-StopHook $holdRepo ([guid]::NewGuid())
...
$h = Invoke-StopHook $holdRepo ([guid]::NewGuid())
$env:QGATE_HOLD = $null
Check 'QGATE_HOLD=1 holds the hook where no marker file was written' `
    (($h.Code -eq 0) -and ($h.Err -match 'QGATE_HOLD')) "code=$($h.Code) $($h.Err)"
$relOut = Invoke-Qgate @('release', '-Root', "`"$holdRepo`"")
$h = Invoke-StopHook $holdRepo ([guid]::NewGuid())
...
. (Join-Path $PSScriptRoot 'gate\detect.ps1')
$holdFile = Join-Path ([IO.Path]::GetTempPath()) "quality-gate-stop-hold-$(Get-PathKey $holdRepo).txt"
Set-Content -Path $holdFile -Value ((Get-Date).AddMinutes(-1).ToString('o')) -NoNewline
$h = Invoke-StopHook $holdRepo ([guid]::NewGuid())
Check 'an expired hold does not keep the gate off' ($h.Code -eq 2) "code=$($h.Code) $($h.Err)"
Check 'an expired hold is swept, not left looking active' (-not (Test-Path $holdFile)) $holdFile
Check 'qgate hold rejects a window outside 1..120 minutes' `
    ((Invoke-Qgate @('hold', '-Minutes', '999', '-Root', "`"$holdRepo`"")) -match '-Minutes must be 1\.\.120') ''
