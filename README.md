...
(`python` определяется, но не проверяется). Имя, которого в репозитории нет, больше не проходит
молча — `[WARN] -Only <name> -- no such stack detected here`: раньше `-Only nonsense` не проверял
ничего и выходил с кодом 0, что читается ровно как чистый прогон.
...
`-Only` с неизвестным стеком предупреждает, `qgate.json` с неизвестным ключом предупреждает,
`qgate.json` пинит gdtoolkit.
42 проверки (часть уходит в `[skip]`, когда нет `GODOT_BIN` или gdtoolkit). Временный каталог самопроверки уникален
