...
## Corpus Check
- 44 files · ~104,508 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 405 nodes · 631 edges · 44 communities (18 shown, 14 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 45 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2f66c673`
...
Cohesion: 0.07
Nodes (35): "Gate is wrong" issue template, git check-ignore exit codes; --stdin batch unusable on Windows, cherry-pick and revert cannot be covered cheaply, Deliberately chosen boundaries (not TODOs), GOTOOLCHAIN=auto unpacking races produce 'missing std package', Measured git hook coverage matrix (git 2.53), Selftest counts 121 online / 115 offline, Coverage measures happy paths; a green suite proves nothing (+27 more)
...
- **Why does `qgate wire (repo wiring, config only)` connect `qgate wire (repo wiring, config only)` to `Claude Code Stop hook (exit code 2)`, `.golangci.yml template`?**
