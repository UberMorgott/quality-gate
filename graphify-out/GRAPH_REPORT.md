...
- **Are the 3 inferred relationships involving `Invoke-CustomStack()` (e.g. with `Get-CustomChecks()` and `Get-PathKey()`) actually correct?**
  _`Invoke-CustomStack()` has 3 INFERRED edges - model-reasoned connections that need verification._
